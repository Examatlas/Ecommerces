...
const ExamBook = () => {
  ...
  const getBooksByCategoryName = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/category/getBooksByCategoryName/${category}`);
  ...
  return (
    <div className="p-6 min-h-screen">
      <h1 className="text-4xl font-bold text-center mb-8 text-blue-700 mt-20">
        Books for {categoryName}
      </h1>

      {books.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 ml-10 mx-auto my-10">
          {books.map((book) => (
            <div
              key={book._id}
...
//     try {
//       const response = await axios.get(`${API_BASE_URL}/category/getBooksByCategoryName/${categoryName}`);
//       console.log("Response:", response.data);
//       setBooks(response.data.books);
//     } catch (error) {
//       console.error("Error fetching books:", error);
